...
/**
 * Gemini's responseSchema is based on OpenAPI 3.0 and does not support
 * the "additionalProperties" keyword. zod-to-json-schema always adds it,
 * so we strip it out recursively before sending the schema to Gemini.
 */
function stripAdditionalProperties(schema) {
    if (Array.isArray(schema)) {
        ...
        Object.values(schema).forEach(stripAdditionalProperties)
    }
    return schema
}
...
function toGeminiSchema(zodSchema) {
    ...
    return stripAdditionalProperties(jsonSchema)
}
...
const interviewReportSchema = z.object({
    ...
    skillGaps: z.array(z.object({
        ...
        severity: z.enum(["low", "medium", "high"]).describe("How important this gap is for the job")
...
module.exports = { generateInterviewReport, generateInterviewReport, generateResumePdf }